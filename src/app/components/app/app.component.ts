...
import { 
	...
	ViewChild, QueryList, ViewChildren
} from '@angular/core';
...
export class AppComponent extends CWidget   { 
	...
	constructor(
		...
		// Listeners
		this.m.se.listenToSel(DNWAdminPanel, (dNode:DNWAdminPanel) => {
			this.se_show = dNode.show;
		});
	}

	setCarousel():DNSlide[] {
		let slides = this.dnSlider.slides;
		return slides;
	}
	...
	click(ev) {
		...
		switch (ui) {
			case "button:editCarousel":

				//let wAdminPanel = this.dm3.select(DNWAdminPanel);
				//this.im.dispatch('[UI Show/Hide] show', { widget:'w/appCarousel', ui: ui, target: wAdminPanel, ctx:'close', sendJSONtoServer:false, serverRequset:null });
...
export class WCarousel extends CWidget { 

	// - X
	@Input() circular: boolean = true;

	temp:string = 'w/carousal';
	...
	ngOnInit() {
		super.ngOnInit();
		this.data = this.m.dm3.select(DNSlider);
	}
	...
	// 	// let ui = ev.getAttribute('name');
	// 	// // let currentSlide:DNSlide = se_slides.find((slide) => {
	// 	// // 	return slide == slide.active;
	// 	// // })

	// 	// this.im.dispatch('[Nav] goTo', { widget:'w/carousel', ui: ui, currentSlider: slide });
	// }

	// // Mechanika Carouselu
	onNextNav(ev) {  // move in derection 1
		let lastPage = (this.page === (this.data.slides.length - 1));
	...
	onPrevNav(ev) { // move in derection 2
		if(this.page !== 0) {
			this.setPage(this.page - 1);
		}
		else if(this.circular) {
			this.setPage(this.data.slides.length - 1);
		}
	}
	...
	setPage(p, enforce?: boolean) {
		if(p !== this.page || enforce) {
		   	...
		   	let currentSlide = this.value[this.page];
		   	currentSlide.active = false;

		   	this.page = p;

		   	let nextSlide = this.value[this.page];
	...
	// // Getter/Setters
	// get totalPages(): number { // vypecet pages
 ...
 //    }
 	...
 	goTo(currentSlide:DNSlide) {

 		// 	let currentSlide:DNSlide = this.data.slides.find((slide) => {
		// 	return slide == slide.active;
		// })
...
@Component({
	selector: 'w-admin-panel',
	templateUrl: './app.component.html',
	styleUrls: ['./app.component.scss'],
	//inputs: ['instance', 'name']
})
export class WAdminPanel extends CWidget { 

	temp:string = 'w/adminPanel';
	data:DNWAdminPanel;
	...
	ngOnInit() {
		...
		super.ngOnInit();
		let dnWAdminPanel = this.m.dm3.select(DNWAdminPanel);
		this.data = this.m.dm3.select(DNWAdminPanel);
	...
	cleareSliderTemp() { // < t/X
		this.data.sliderTemp = new DNSlider();
	}

	updateSlider():DNSlider {
		let slider = this.m.dm3.select(DNSlider);
		//slider = { slider, ...this.sliderTemp };
		
		slider.subHeader = this.data.sliderTemp.subHeader;
		return slider;
	}
	...
	click(ev, modal) {
		...
		switch (ui) {
			case "ui/button:close":
				//this.im.dispatch('[UI Show/Hide] show', { widget:'w/adminPanel', ui: ui, target: this.data, ctx:'close', sendJSONtoServer:false, serverRequset:null });
			...
			case "ui/button:save":
				// let serverRequest = {
				// 	DNode: sliderTemp.Type,
				// 	id: sliderTemp.id,
				// 	cfg: sliderTemp.getConfig(),
				// };
				this.data.show = false;
				this.saved = true;
				modal.approve();
...
@Component({
	selector: 'w-tiles-editor',
	templateUrl: './app.component.html',
	styleUrls: ['./app.component.scss'],
	//inputs: ['instance', 'name']
})
export class WTilesEditor extends CWidget { 

	@Input() data:DNWAdminPanel;
	temp:string = 'w/tilesEditor';
	//sliderTemp:DNSlider;
	...
	constructor(m:Managers) { // < private _sel:Selectors
		...
		// Listeners
		// this.m.se.listenToSel(DNWAdminPanel, (dNode:DNWAdminPanel) => {
		// 	this.se_tiles = dNode.sliderTemp.slides;
		// });
	}
...
@Component({
	selector: 'w-tile-item',
	templateUrl: './app.component.html',
	styleUrls: ['./app.component.scss'],
	//inputs: ['instance', 'name']
})
export class WTileItem extends CWidget { 

	@Input() dnWAdminPanel:DNWAdminPanel;
	@Input() data:DNSlide;

	temp:string = 'w/tileItem';
	//slide:DNSlide;
	...
	click(ev, action) {
		...
		switch (ui) {
			case "ui/dropdownMenu:delete":
				this.delete();
				break;
			case "ui/dropdownMenu:moreActions":
				let wAdminPanel;
				...
				switch (action) {
					...
					default:
						console.log('no action for ui/dropdownMenu:moreActions difine');
						break;
				}

				wAdminPanel = this.sm.dm.select(DNWAdminPanel);
				this.im.dispatch('[UI Open/Close] open', { widget:'w/tileItem', ui: ui, targetData: wAdminPanel, target: this.data });
				break;
			default:
				console.log('no ui element difine');
				break;
	...
	delete() {
		let slideToDeleteIndex = this.dnWAdminPanel.sliderTemp.slides.indexOf(this.data);
		this.dnWAdminPanel.sliderTemp.slides.splice(slideToDeleteIndex, 1);
	}
}

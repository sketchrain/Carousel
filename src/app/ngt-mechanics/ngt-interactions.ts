...
import { Task, WTask, DNWAdminPanel, DNSlide, DNSlider } from './ngt-states';
import { WAdminPanel } from '../components/app/app.component';
...
export class Interactions extends Category {
	...
	interactionsCarousel() {
		...
		// < w/Carousel
		// stream = this.im.action('[Nav] goTo', { currentSlide:<DNSlide> }) // < exist
		// 	.toOutput((sig:Signal, p:any) => {
		// 		let currentSlide:DNSlide =  p.currentSlide;
		// 		window.open(currentSlide.linkTo.url, currentSlide.linkTo.openIn ? '_blank'  : '_self' );
		// 	})
		
		// < a/AppCorousel
		let stream = this.im.action('[UI Show/Hide] show', { target:<any> null, ctx:<string> '', sendJSONtoServer:<boolean> false, serverRequset:<any> null }) // < a/show/hide (w/adminPanel) - a/close (w/adminPanel) - a/saveSetting (carousel)
			// update local carousal data model
			.operation('open-close-save', (signal:Signal, p:any) => { // < a/close (w/adminPanel)

				if(!p.sendJSONtoServer) {
					let target:DNWAdminPanel = p.target;

					switch (true) {
						case target instanceof DNWAdminPanel:
							
							if(!target.show) {
								target.show = true;
								target.widget.setupSliderTemp();
								//signal.addToOutput('[State] changed state', { changedState: updatedSlider }, stream.am);
							}
							else {
								
								switch (p.ctx) {
									case 'close':
									target.widget.cleareSliderTemp();
									break;
									
									case 'save':
									let updatedSlider:DNSlider = target.widget.updateSlider();
									target.widget.cleareSliderTemp();
									//signal.addToOutput('[State] changed state', { changedState: updatedSlider }, stream.am);
									break;
								}
			...
			// send carousal setting to server in JSON 
			// .XasyncOperation('', (action:Signal, p:any) => { // overeni uzivatele na serveru;
			// 	return this.server.update(action.payload.serverRequest.DNode, action.payload.serverRequest.id, action.payload.serverRequest.cfg, (res:any) => {
			// 		return this.im.utils.modifyAction(action, { res: res });
			// 	});
			// })
			// .setState((signal:Signal, p:any) => {
			// 	let target:WAdminPanel = this.sm.dm.selectByType(p.target);
			// 	target.show = target.show ? true : false;

			// 	// server response evaluation
			// 	if(p.sendJSONtoServer && target instanceof DNAdminPanel && !target.show && p.ctx == 'save' ) {
			// 		let carouselApp:DNCarouselApp = this.sm.dm.selectByType(DNCarouselApp);

			// 		carouselApp.showServerResponse = true;
			// 		carouselApp.serverResponseText = signal.payload.res ? 'carousel JSON setting object was succesfuly send to server' : 'faild to send carousel JSON setting object to server';
			// 		signal.addToOutput('[State] changed state', { changedState: carouselApp }, stream.am);

			// 		console.log('Carousel JSON setting object: ', signal.payload.res.data);
			// 	}
				

			// 	signal.addToOutput('[State] changed state', { changedState: target }, stream.am);
			// 	return signal;
			// });

		// // w/tileEditor
		// stream = this.im.action('[CRUD] create slide/tile', { targetData:<DNWAdminPanel> })
		// 	.setState((action:Signal, p:any) => {
		// 		let targetData:DNWAdminPanel = this.sm.dm.selectByType(p.targetData); // target data

		// 		let createdSlide = new Slide();
		// 		targetData.sliderTemp.slides.push(createdSlide);

		// 		return this.am.utils.modifyAction(action, { changedState: targetData });
		// 	})
		// 	.toOutput('[State] changed state', (prevAPay:any)=> { return { changedState: prevAPay.changedState } });

		// // w/tileItem
		// stream = this.im.action('[CRUD] delete slide/tile', { targetData:<DNWAdminPanel>, target:<DNSlide> })
		// 	.setState((action:Signal, p:any) => {
		// 		let targetData:DNWAdminPanel = this.sm.dm.selectByType(p.targetData); // target data

		// 		let slideToDeleteIndex = targetData.sliderTemp.slides.indexOf(target);
		// 		targetData.sliderTemp.slides.splice(slideToDeleteIndex, 1);
		...
		// // w/tileItem
		// stream = this.im.action('[UI Open/Close] open', { target:<DNWTileItem> })
		// 	.setState((action:Signal, p:any) => {
		// 		let target:DNWTileItem = p.target;

		// 		switch (target) {
					
		// 			case instanceof DNWTileItem:
		// 			target.open = target.open ? false : true;
		// 			return this.am.utils.modifyAction(action, { changedState: target });;
